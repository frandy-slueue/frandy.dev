...
export default function AdminResume() {
  ...
  return (
    <div className="resume">
          ...
          {resumes.map((r) => (
            <div key={r.id} className={`resume__item ${r.is_active ? "active" : ""}`}>
              ...
              <div className="resume__actions">
                <a href={r.file_url} target="_blank" rel="noopener noreferrer" className="admin-btn-ghost">View</a>
                {!r.is_active && (
                  <button className="admin-btn-ghost" onClick={() => handleActivate(r.id)}>Set Active</button>
                )}
                <button className="admin-btn-danger" onClick={() => handleDelete(r.id)}>Delete</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <style jsx>{`
        .resume__header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 2rem; }
        .resume__header h1 { font-family: var(--font-display); font-size: 2rem; margin: 0; }
        .resume__header p { color: var(--color-text-muted); margin: 0.25rem 0 0; font-size: 0.875rem; }
        ...
        .resume__list { display: flex; flex-direction: column; gap: 0.75rem; }
        .resume__item { display: flex; align-items: flex-start; flex-wrap: wrap; gap: 1rem; background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 8px; padding: 1.25rem; transition: border-color 0.2s; }
        .resume__item.active { border-color: var(--color-accent); }
        .resume__icon { font-size: 2rem; flex-shrink: 0; }
        .resume__info { flex: 1; }
        .resume__filename { font-size: 0.95rem; color: var(--color-text); font-family: var(--font-mono); }
        .resume__meta { font-size: 0.8rem; color: var(--color-text-muted); margin-top: 0.25rem; display: flex; align-items: center; gap: 0.75rem; }
        .resume__badge { background: rgba(0,255,128,0.1); color: #00ff80; font-size: 0.7rem; padding: 0.2rem 0.6rem; border-radius: 999px; font-family: var(--font-mono); }
        .resume__actions { display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap; width: 100%; }
        .empty { padding: 3rem; text-align: center; color: var(--color-text-muted); display: flex; flex-direction: column; align-items: center; gap: 1rem; }
        @media (max-width: 768px) {
          .resume__header { flex-direction: column; gap: 1rem; align-items: flex-start; }
          .resume__filename { font-size: 0.8rem; word-break: break-all; }
        }
      `}</style>
    </div>
  );
