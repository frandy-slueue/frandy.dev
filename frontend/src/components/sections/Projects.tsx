...
const FILTER_TABS: TabItem[] = [
  "All", "Frontend", "Backend", "Fullstack", "Python", "JavaScript",
].map((label) => ({ label }));
...
function getShapeLayout(count: number) {
  return Array.from({ length: count }, (_, i) =>
    SHAPES[Math.floor(Math.random() * Math.min(4, SHAPES.length))]
  );
}

// ── Project cell ─────────────────────────────────────────────────────
// Hover effects handled entirely in CSS — no useState for hover needed.
function ProjectCell({
  ...
  return (
    <motion.article
      ...
      onClick={onClick}
      // gridColumn/gridRow are truly dynamic (data-driven shape)
      style={{
    ...
    >
      ...
      {project.thumbnail_url && (
        <div
          className="project-cell__thumb"
          style={{ backgroundImage: `url(${apiBase}${project.thumbnail_url})` }}
          aria-hidden
        />
      )}
      ...
      <div className="project-cell__resting">
        <span className="project-cell__num">
          {String(project.sort_order + 1).padStart(2, "0")}
        </span>
        <p className="project-cell__title">{project.title}</p>
      </div>

      {/* Hover overlay — shown via CSS .project-cell:hover */}
      <div className="project-cell__overlay" aria-hidden={false}>
        <div>
...
function ProjectDetail({
  ...
  return (
    ...
    >
      <button className="project-detail__close" onClick={onClose} aria-label="Close">
        <X size={16} />
      </button>
      ...
      <div className="project-detail__section">
        ...
        <div className="project-detail__tags">
          {project.stack_tags.map((tag) => (
            <span key={tag} className="stack-tag stack-tag--lg">{tag}</span>
          ))}
        </div>
      </div>
...
export default function Projects() {
  ...
  // Morph grid shapes every 5–8 seconds
  useEffect(() => {
    const schedule = () => {
